...
import numpy as np
import torch
from numpy.polynomial import Legendre, Polynomial  # orthogonal polynomials
from scipy.interpolate import griddata


def assemble_design_matrix(Y, ho=2, basis=Legendre, eps=1.0):
    M, D = Y.shape
    Y_d = torch.zeros(M, ho + 1, D, dtype=Y.dtype, device=Y.device)
    for d in range(D):
...
def reorder_variables(
    theta,  # N x K Torch tensor where N is the number of grains, K is the number of features
    ...
    I1 = [
        indices
        for indices in itertools.product(range(ho_end + 1), repeat=D)
        if sum(indices) < ho_end + 1
    ]
    I2 = [
        indices
        for indices in itertools.product(range(ho_start + 1), repeat=D)
        if sum(indices) < ho_start + 1
    ]
...
def calculate_moments_from_data(
    grain_map,
    Y,
    ho=2,
    basis=Polynomial,
):
    N = (grain_map.max() + 1).item()
    M, D = Y.shape
    DM = assemble_design_matrix(Y, ho=ho, basis=basis)
    _, K = DM.shape
    moments = torch.zeros(N, K)
    for k in range(K):
        moments[:, k] = torch.bincount(grain_map, DM[:, k], minlength=N)
    return moments
...
def convert_from_lr_to_phys(theta, ho=1):
    ...
    if ho == 2:
        ...
        phys_var[:, 5] = (
            theta[:, 0]
            + (phys_var[:, 3] * theta[:, 3] + phys_var[:, 4] * theta[:, 1]) / 2.0
        )
    if ho > 2:
        print("No conversion implemented, returning zeros")
    return phys_var
...
def convert_from_phys_to_lr(phys_var, ho=1):
    ...
    if ho == 2:
        theta[:, 0] = phys_var[:, -1] - (
            phys_var[:, 0] * phys_var[:, 3] ** 2
            + 2.0 * phys_var[:, 1] * phys_var[:, 3] * phys_var[:, 4]
            + phys_var[:, 2] * phys_var[:, 4] ** 2
        )
        theta[:, 1] = 2.0 * (
            phys_var[:, 1] * phys_var[:, 3] + phys_var[:, 2] * phys_var[:, 4]
        )
        theta[:, 2] = -phys_var[:, 2]
        theta[:, 3] = 2.0 * (
            phys_var[:, 0] * phys_var[:, 3] + phys_var[:, 1] * phys_var[:, 4]
        )
...
def physical_heuristic_guess(grain_map, Y, ho=2):
    _, D = Y.shape
    moments = calculate_moments_from_data(grain_map, Y, ho=ho)
    if ho > 2:
...
def convert_theta_between_bases(
    theta, ho, D, basis_start=Polynomial, basis_end=Legendre
):
    j = 0
...
def gridify_Y_I(Y, grain_map, domain, pixel_params, color_by=None):
    x = Y[:, 0]
    y = Y[:, 1]
    z = color_by[grain_map] if color_by is not None else grain_map
    # define grid
