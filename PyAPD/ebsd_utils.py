...
# def load_setup_from_EBSD_data(file = "../../data/2D_basic_example/EBSD_example_2D_data.txt",
#                               device = "cuda" if torch.cuda.is_available() else "cpu",
#                               dt = torch.float32,
#                               normalise_matrices = True):
#     """ 
#     Load geometric setup from some EBSD data. The data is located in `/data`. The data format is assumed to be such each line corresponds to one grain, e.g.
...
#     Input:
#     file - location of the file, should be of the form "../data/..." to load some data from the data folder (optional, default is "../data/EBSD_example_2D_data.txt" )
#     """
#     #EBSD = np.loadtxt("../data/EBSD_data.txt")
#     EBSD = torch.from_numpy(np.loadtxt(file)).to(device=device, dtype=dt)
#     X = EBSD[:,1:3]
#     TV = EBSD[:,3]
#     if normalise_matrices:
#         ratios = EBSD[:,4] / EBSD[:,5]
#         major_axes = torch.sqrt(ratios)
#         minor_axes = 1/major_axes
#         thetas = EBSD[:,6]*np.pi / 180
#     else:
#         major_axes = EBSD[:,4]
#         minor_axes = EBSD[:,5]
#         thetas = EBSD[:,6]*np.pi / 180
